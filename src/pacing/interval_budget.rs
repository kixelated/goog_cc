...
impl IntervalBudget {
  ...
  self.bytes_remaining = std::cmp::min(std::cmp::max(-self.max_bytes_in_budget, self.bytes_remaining),
  ...
  pub fn IncreaseBudget(&mut self, delta_time_ms: i64) {
    ...
    if self.bytes_remaining < 0 || self.can_build_up_underuse {
      ...
      self.bytes_remaining = std::cmp::min(self.bytes_remaining + bytes, self.max_bytes_in_budget);
    } else {
    ...
    pub fn UseBudget(&mut self, bytes: usize) {
      self.bytes_remaining = std::cmp::max(self.bytes_remaining - bytes as i64,
      -self.max_bytes_in_budget);
    }
