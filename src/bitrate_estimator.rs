...
impl BitrateEstimator {
  pub fn Update(&mut self, at_time: Timestamp, amount: DataSize, in_alr: bool) {
   ...
   let bitrate_sample_kbps: f64 = self.UpdateWindow(at_time.ms(), amount.bytes(),
   ...
   // symmetry.
   let sample_uncertainty: f64 =
       scale * (self.bitrate_estimate_kbps - bitrate_sample_kbps).abs() /
       (self.bitrate_estimate_kbps +
        bitrate_sample_kbps.max(
                 self.uncertainty_symmetry_cap.kbps_float()));
   ...
   self.bitrate_estimate_kbps =
       self.bitrate_estimate_kbps.max(self.estimate_floor.kbps_float());
  ...
  fn UpdateWindow(&mut self, now_ms: i64, bytes: i64, rate_window_ms: i64, is_small_sample: &mut bool) -> f64 {
