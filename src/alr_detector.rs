...
const kEstimatedBitrateBps: i64 = 300000;
...
fn ProduceTraffic(&mut self) {
  ...
  let usage_percentage= self.usage_percentage.unwrap();
    const kTimeStepMs: i64 = 10;
    let mut t: i64 = 0;

    while t < interval_ms {
      *self.timestamp_ms += kTimeStepMs;
      self.alr_detector.OnBytesSent((kEstimatedBitrateBps * usage_percentage *
                                     kTimeStepMs / (8 * 100 * 1000)) as usize,
                                 *self.timestamp_ms);
                                 t += kTimeStepMs;
    }
    let remainder_ms: i64 = interval_ms % kTimeStepMs;
    if remainder_ms > 0 {
      *self.timestamp_ms += kTimeStepMs;
      self.alr_detector.OnBytesSent((kEstimatedBitrateBps * usage_percentage *
                                     remainder_ms / (8 * 100 * 1000)) as usize,
                                 *self.timestamp_ms);
    }
  }
}
...
fn AlrDetection() {
  ...
  assert!(alr_detector.GetApplicationLimitedRegionStartTime().is_none());
  ...
  assert!((alr_detector.GetApplicationLimitedRegionStartTime().is_some()));
